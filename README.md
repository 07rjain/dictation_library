...
The Pages deployment installs the declared stable npm package and serves it through content-addressed application and ESM assets stamped with the deployment commit. This prevents a browser cache from combining incompatible website and library versions. Deployment fails if the declared package version is not available from npm.
